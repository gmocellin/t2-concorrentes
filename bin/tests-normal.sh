...
do
	...
	for((i=0 ;i<10;i++))
	do
		(/usr/bin/time -f "\n%e" make run ARGS=\\"images/$test_file\\") 2>> results/tmp-normal-$test_file 1>> results/result-normal-$test_file
		total_time_single=$(python -c "print ($total_time_single + $(tail -n 1 results/tmp-normal-$test_file))")
